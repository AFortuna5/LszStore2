...
export default async function FinancePage() {
  ...
  const [stores, paid, failed, refunded, disputed, failedWebhooks] = await Promise.all([
  ...
  const payouts = env.stripeSecretKey ? await Promise.all(stores.map(async (store) => {
    try {
      const result = await stripeClient().payouts.list({ limit: 10 }, stripeRequestOptions(store.stripeAccountId));
      return { storeId: store.id, count: result.data.length, amount: result.data.filter((payout) => payout.status === "paid").reduce((sum, payout) => sum + payout.amount, 0) };
    } catch {
      return { storeId: store.id, count: 0, amount: 0 };
    }
  })) : [];
  const payoutsByStore = new Map(payouts.map((payout) => [payout.storeId, payout]));
  ...
  return <SiteShell><StripeConnectPanel canManageCommission={session.role === "ADMIN"} /><section className="bg-black pb-16"><div className="container mx-auto max-w-4xl px-4 md:px-6">
    <h2 className="font-montserrat text-2xl font-black uppercase text-white">Visao da plataforma</h2>
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Vendido", `R$ ${totalSold.toFixed(2)}`], ["Comissoes", `R$ ${(totalFees / 100).toFixed(2)}`], ["Pedidos pagos", paid], ["Falhos / expirados", failed], ["Reembolsos", refunded], ["Disputas", disputed], ["Webhooks com falha", failedWebhooks]].map(([label, value]) => <div key={String(label)} className="rounded border border-border bg-dark-blue p-4"><p className="text-xs uppercase text-silver">{label}</p><p className={`mt-2 text-xl font-bold ${label === "Webhooks com falha" && Number(value) > 0 ? "text-red-300" : "text-white"}`}>{value}</p></div>)}</div>
    <div className="mt-8 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-silver"><tr><th className="p-3">Loja</th><th>Status</th><th>Pagamentos</th><th>Repasses Stripe</th><th>Pedidos</th></tr></thead><tbody>{stores.map((store) => { const payout = payoutsByStore.get(store.id); return <tr key={store.id} className="border-t border-border"><td className="p-3 text-white">{store.name}</td><td>{store.stripeAccountStatus}</td><td>{store.stripeChargesEnabled ? "Ativo" : "Bloqueado"}</td><td>{store.stripePayoutsEnabled ? `${payout?.count ?? 0} repasses · R$ ${((payout?.amount ?? 0) / 100).toFixed(2)}` : "Pendente"}</td><td>{store._count.orders}</td></tr>; })}</tbody></table></div>
