...
export async function createPaymentSession(orderId: string) {
  ...
  if (!accountId || !order.store.stripeChargesEnabled || order.store.stripeAccountStatus !== "ACTIVE") throw new Error("Loja nao habilitada para pagamentos");
  ...
  const fee = platformFee(totalCents, percentage);
  ...
  const checkout = await stripe.checkout.sessions.create({
    ...
    payment_intent_data: { application_fee_amount: fee, metadata: { orderId: order.id, storeId: order.storeId } },
  }, stripeRequestOptions(accountId, `checkout_${order.id}`));
...
async function applySession(session: Stripe.Checkout.Session, accountId: string, action: "completed" | "succeeded" | "failed" | "expired") {
...
async function applyIntent(intent: Stripe.PaymentIntent, accountId: string, succeeded: boolean) {
...
async function applyReversal(paymentIntentId: string, amount: number, accountId: string, type: "REFUND" | "DISPUTE", reference: string, chargeId?: string) {
  return prisma.$transaction(async (tx) => {
...
async function handleStripeEvent(event: Stripe.Event) {
  ...
  if (!accountId) throw new Error("Evento de pagamento sem conta conectada");
  switch (event.type) {
