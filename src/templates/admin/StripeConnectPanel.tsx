...
type StripeStatus = { storeId: string; storeName: string; connected: boolean; chargesEnabled: boolean; payoutsEnabled: boolean; detailsSubmitted: boolean; onboardingCompleted: boolean; status: string; requirements: string[]; commissionPercentage: number; webhookDestinationsConfigured: number };

export default function StripeConnectPanel({ canManageCommission = false }: { canManageCommission?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) setError(payload.error ?? "Nao foi possivel consultar a Stripe"); else { setData(payload); setCommission(String(payload.commissionPercentage)); }
    setLoading(false);
  }, []);
  useEffect(() => {
    let active = true;
    fetch("/api/stripe/connect/status", { cache: "no-store" })
      ...
      .then(({ ok, payload }) => { if (active) { if (ok) { setData(payload); setCommission(String(payload.commissionPercentage)); } else setError(payload.error ?? "Nao foi possivel consultar a Stripe"); } })
  ...
  async function saveCommission() {
    ...
    const response = await fetch(`/api/admin/stores/${data.storeId}/commission`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ percentage: Number(commission) }) });
    const payload = await response.json();
    if (!response.ok) setError(payload.error ?? "Nao foi possivel atualizar a comissao");
    else setData({ ...data, commissionPercentage: payload.commissionPercentage });
    setLoading(false);
  }
  ...
  return <section className="min-h-screen bg-black py-12"><div className="container mx-auto max-w-4xl px-4 md:px-6">
    ...
    {error && <div className="mt-6 rounded border border-red-500/50 bg-red-500/10 p-4 text-red-200"><p>{error}</p>{actionUrl && <a href={actionUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex rounded bg-white px-4 py-2 text-sm font-bold uppercase text-black hover:bg-neon-blue">Resolver na Stripe</a>}</div>}
    <div className="mt-8 rounded-lg border border-border bg-dark-blue p-6">
      {loading && !data ? <p className="text-silver">Consultando status seguro...</p> : data && <>
        ...
        <p className={`mt-4 rounded border p-4 text-sm ${data.webhookDestinationsConfigured >= 2 ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-100" : "border-red-500/40 bg-red-500/10 text-red-100"}`}>{data.webhookDestinationsConfigured >= 2 ? "Webhooks da conta principal e do Connect configurados." : "Pendente: configure os dois segredos de webhook no Vercel."}</p>
        <div className="mt-6 flex flex-wrap items-center gap-3 text-sm text-silver"><span>Comissao da plataforma: <strong className="text-white">{data.commissionPercentage}%</strong></span>{canManageCommission && <><input aria-label="Comissao da plataforma" value={commission} onChange={(event) => setCommission(event.target.value)} type="number" min="0" max="99.99" step="0.01" className="w-24 rounded border border-border bg-black px-3 py-2 text-white outline-none focus:border-neon-blue" /><button type="button" onClick={() => void saveCommission()} disabled={loading} className="rounded border border-neon-blue px-3 py-2 text-xs font-bold uppercase text-neon-blue hover:bg-neon-blue hover:text-black disabled:opacity-50">Salvar comissao</button></>}</div>
