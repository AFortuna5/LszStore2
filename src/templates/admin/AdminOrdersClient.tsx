...
type Order = {
  id: string; status: string; paymentStatus: string; paymentMethod: string; paymentProvider: string; subtotal: number; discountAmount: number; couponCode: string | null; total: number;
...
export default function AdminOrdersClient() {
  ...
  async function refundOrder(order: Order) {
    ...
    const data = await response.json();
    if (!response.ok) { setMessage(data.error ?? "Nao foi possivel solicitar o reembolso"); return; }
    setMessage("Reembolso solicitado com seguranca. O status sera confirmado pelo webhook da Stripe.");
    await load();
  }
  ...
  return <section className="min-h-screen bg-black py-12"><div className="container mx-auto px-4 md:px-6">
    ...
    <div className="mt-6 space-y-5">{loading ? <p className="text-silver">Carregando...</p> : visible.length === 0 ? <p className="rounded border border-border bg-dark-blue p-8 text-center text-silver">Nenhum pedido encontrado.</p> : visible.map((order) => <OrderCard key={order.id} order={order} onSave={updateOrder} onRefund={refundOrder} />)}</div>
  </div></section>;
}

function OrderCard({ order, onSave, onRefund }: { order: Order; onSave: (order: Order, status: string, tracking: string) => Promise<void>; onRefund: (order: Order) => Promise<void> }) {
